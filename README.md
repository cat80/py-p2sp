...
对项目做一些介绍,p2sp的架构模式。在server端采用依赖注册的方式，解耦合。使用把ChatServer,MessageHandler，services,repository全部独立出来，减少依赖。同时增加专门的连接管理对象，管理所有的在线连接和消息发送，使每个模块都尽量独立，减少依赖。
...
2.好友端
    ...
    - 给好友发送消息（支持离线消息）
3. 离线消息
   当好友不在线时，消息并不会丢失，会保存起来等好友上线时（即下次登陆成功后）发会好友。
4. 管理端
    有完善的管理。
    - 广播消息。管理员能给所有在线用户广播消息。
    - 禁用用户。管理员能禁用用户。
    - 管理员能解禁用户。

未实现的功能：
    1. 群组功能。用户可以创建群组，解散群组、邀请好友、群聊。
    2. 文件传输功能。这个通过p2sp是很好实现的。当时考虑p2p实现，使用UDP的方式。甚至还想会牵涉到UDP打洞（stun,turn)的问题。
    3.一个桌面或者手机APP交互端。做这个确实有吸引力，但考虑到周期和关注点还是放弃了。

总之，这个项目让我深入理解了，网络编程和asyncio。为根据alt-coin的p2p部分戴